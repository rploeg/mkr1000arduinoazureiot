...
## Arduino
use the above .ino file as baseline to send messages with your MKR1000 WiFi. Create arduino_secrets.h file to store the links to your enviornment. 
...
## Azure IoT Hub
Create a new IoT device in the Azure IoT Hub, when creating the new devive don't use the SAS authentication, but the self signed certificate authentication. Paste the SHA key from the previous step into the primary and secondary key of your device.

## Connect & Run
Connect the DHT22 sensor to the board of your MKR1000 :
...
Run the code on Arduino with the device ID you have created in the secrets file. Check with your serial port if the data is send to the Azure IoT Hub. If yes, check with Azure IoT Explorer if the data is received in the IoT Hub.
